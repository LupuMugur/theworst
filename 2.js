...
window.setInterval(function() {
    ...
    body.style.backgroundColor = COLORS[index][pos]

    pos = Math.random() > 0.5 ? 0 : 1;
    index = Math.floor(Math.random() * 5);
    body.style.color = COLORS[index][pos]

    for (let cpt = 0; cpt < elements.length; cpt++) {
        pos = Math.random() > 0.5 ? 0 : 1;
        index = Math.floor(Math.random() * 5);
        if (elements[cpt] !== 'undefined') {
            elements[cpt].style.backgroundColor = COLORS[index][pos];
    
            pos = Math.random() > 0.5 ? 0 : 1;
            index = Math.floor(Math.random() * 5);
            elements[cpt].style.color = COLORS[index][pos];
    
            elements[cpt].style.fontSize = Math.floor(Math.random() * 32) + 44 + "px";
        }    
    }  
...
document.addEventListener('keydown', function(event) {
    if (event.key === 'Enter') {
        document.getElementById('modal_thing').style.display = 'block'
    }
})
